...
const BRIEF_PROMPT_LABELS: Record<keyof BriefResult["prompts"], string> = {
  codex: "Codex",
  claude: "Claude",
  chatgpt: "ChatGPT",
  cursor: "Cursor",
...
export function exportBriefMarkdown(result: BriefResult): string {
  const date = new Date().toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" });
  const lines: string[] = [result.briefMarkdown.trim(), ``, `## AI Build Prompts`, ``];

  for (const key of Object.keys(result.prompts) as (keyof BriefResult["prompts"])[]) {
    lines.push(`### ${BRIEF_PROMPT_LABELS[key]}`, ``, "```text", result.prompts[key].trim(), "```", ``);
  }
